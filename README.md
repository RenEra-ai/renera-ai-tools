...
| `codex-claude` | Use Codex (GPT-5.x) as an architect & reviewer in the Claude Code dev loop; `/codex-issue` wraps the repo's own workflow in a fully autonomous architect → implement → review → PR loop, composing with a Claude Code Workflow when present (`/codex-compose-setup` makes any repo composition-ready) | 1.6.0 |
