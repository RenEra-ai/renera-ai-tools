...
| `agents/codex-architect.md` | **New (thin)** | Tools: **`Bash`, `Read`, `Write`** (no `Task`/`Grep`/`Glob`; `Write` builds the Codex prompt file inject-safely — issue text may contain backticks/`$()`; `Bash` for any search). Runs the ephemeral `scripts/plan-round.mjs` (which writes `--out .codex/plans/issue-N.md`), returns the verbatim Codex design-plan text + status. Transcriber, not author — fails loud if Codex emits no usable plan. |
...
| `agents/codex-reviewer.md` | **Keep, adapt** | Already thin + ephemeral (`scripts/review-round.mjs`). **Drop the dead `Grep`/`Glob`** from its `tools:` (non-functional in subagents) → `tools: Bash, Read, Write` (`Write` builds the review prompt file inject-safely). Adapt to take *plan text + changed files*, return structured `{verdict, reviewedFiles[], findings[]}` with the deterministic last-line `VERDICT:` parse. |
