...
STEP B — FIRST assert a clean tracked tree at handoff (the repo workflow returned ready_to_land, so its work is committed): if \`git status --porcelain --untracked-files=no\` is NON-empty, do NOT proceed — return ok=false with detail beginning "BLOCKED: dirty tracked tree at fix handoff" (a pre-existing tracked modification must not be swept into the fix commit by STEP E's \`git add -u\`). THEN snapshot the pre-existing untracked files to a SCRATCH FILE that survives across separate Bash calls (a shell variable would NOT persist — you run later steps in new shells). Run it FROM THE REPO ROOT so the untracked list is complete and root-relative, matching STEP E: \`cd "$(git rev-parse --show-toplevel)" && git ls-files --others --exclude-standard > "$(git rev-parse --git-path codex-wrap-pre.txt)"\`. (\`git rev-parse --git-path\` — NOT a hard-coded \`.git/...\` — resolves the scratch file correctly in a linked worktree (where \`.git\` is a file) or from a subdirectory; it lives inside the git dir and is never committed.) STEP E excludes these so pre-existing untracked files aren't swept into the squash. THEN apply the fix the repo's way: follow the repo's documented dev procedure / its developer agent's discipline (you can't Task-dispatch that agent — apply its rules directly), otherwise implement the fix per its CLAUDE.md. Apply receiving-code-review discipline: verify each finding, fix only what is genuinely wrong, push back on the rest.
...
\`\`\`bash
cd "$(git rev-parse --show-toplevel)"                  # stage from the repo root: paths match Step B
PRE="$(git rev-parse --git-path codex-wrap-pre.txt)"   # same git-resolved scratch path as Step B
git add -u                                             # tracked edits/deletions from the fix
comm -23 <(git ls-files --others --exclude-standard | sort) <(sort "$PRE") \\
