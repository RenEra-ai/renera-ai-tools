...
STEP B — FIRST assert a clean tracked tree at handoff (the repo workflow returned ready_to_land, so its work is committed): if \`git status --porcelain --untracked-files=no\` is NON-empty, do NOT proceed — return ok=false with detail beginning "BLOCKED: dirty tracked tree at fix handoff" (a pre-existing tracked modification must not be swept into the fix commit by STEP E's \`git add -u\`). THEN capture PRE = the currently-untracked files: \`git ls-files --others --exclude-standard\` (one path per line; may be empty) — you will exclude these from the commit so pre-existing untracked files aren't swept in either. THEN apply the fix the repo's way: follow the repo's documented dev procedure / its developer agent's discipline (you can't Task-dispatch that agent — apply its rules directly), otherwise implement the fix per its CLAUDE.md. Apply receiving-code-review discipline: verify each finding, fix only what is genuinely wrong, push back on the rest.
...
STEP E — STAGE precisely, then GATE + commit. Do NOT use \`git add -A\` (it would sweep the PRE files from Step B into the squash). Stage exactly the fix delta: \`git add -u\` (tracked edits/deletions), then add ONLY the new files you created that are NOT in PRE — compute \`comm -23 <(git ls-files --others --exclude-standard | sort) <(printf '%s\\n' "$PRE" | sort)\` (or equivalent) and \`git add --\` those paths. If AND ONLY IF the repo's review/QA gate(s) reported clean (Step C) AND tests are green (Step D): \`git commit -m\` with a short one-line message (no "Claude Code"), then return ok=true. In detail, LABEL each required gate as "natively run" or "replayed inline" (per STEP C), name the agent/command for each, and ECHO the exact test command + last lines of its output.
